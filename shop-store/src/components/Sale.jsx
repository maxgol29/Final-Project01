            ...
            {phones.map((phone, index) => (
              <div className='border-stone-400 border-[1px] border-solid h-[350px] my-10 shadow-lg'>
                <div className='rounded-t-md flex justify-center items-center flex-nowrap bg-white'>
                  <img src={phone.images[0][0]} alt='' className='h-50 w-50 max-w-52 max-h-52 p-10 transition ease-in-out delay-50 hover:scale-125 duration-500'/>
                </div>
                <div className='flex flex-col justify-start  gap-4 p-4 bg-white font-normal text-xl'>
                  <p>{phone.label + ' ' + phone.color[0].toLocaleUpperCase()}</p>
                  <p className='text-purple-700'>${phone.price[0]}.00</p>
                </div>
              </div>
            ))}
            </Slider>
